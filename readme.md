...
* differentiable handling of visible self intersections
* GPU acceleration
...
###Windows
	...
	pip install git+https://git.repo/martinResearch/DEODR.git
...
Example of fitting a hand mesh to a depth sensor image [*DEODR/examples/depth_image_hand_fitting*](PyDEODR/examples/depth_image_hand_fitting)
 ![animation](./images/python_depth_hand.gif)

Example of fitting a hand mesh to a RGB sensor image [*DEODR/examples/depth_image_hand_fitting*](PyDEODR/examples/rgb_image_hand_fitting)
...
For this example you will also need to download the automatic differentiation toolbox from https://github.com/martinResearch/MatlabAutoDiff 
...
This code implements the core of differentiable renderer described in [1] and has been mostly written in 2008-2009 during my PhD at Ecole Centrale de Paris under the supervision of Nikos Paragios. It is anterior to OpenDR and is the first differentiable renderer to appear in the litterature.
It renders a set of triangles with texture bilinearly interpolated and shaded or with interpolated RGB colour. In contrast with most renderers, the rendered image is differentiable with respect to the vertices positions even along occlusion boundaries using a differentiable antialiasing method called *Discontinuity-Edge-Overdraw* [2] that progressively blends the colour of the front triangle with the back triangle along occlusion boundaries. 
This allows us to compute the derivatives of the rendering function. 
This code does not provide explicitly the sparse Jacobian of the rendering function (like done in [3]) but provides the vector-Jacobian product operator, the backward function in PyTorch and backprop function in Tensorflow.

![latex: \large ~~~~~~~~~~~~~~$\sum_{ij} D(i,j)(\partial I(i,j)/\partial V_k)$~~~~~~~~~~~~~~~~~~~~~~~(1)](./images/backoperator1.svg)

with I(i,j) the intensity of the pixel at location i,j , V<sub>k</sub> the 2D position of the kth vertex of the projected mesh or set of triangles.
For triangles with linearly interpolated colours it also provides

![latex: \large ~~~~~~~~~~~~~~$\sum_{ij} D(i,j)(\partial I(i,j)/\partial C_k)$](./images/backoperator2.svg)

With C<sub>k</sub> the color of the kth vertex.
For triangles with texture it provides 

![latex:\large ~~~~~~~~~~~~~~ $\sum_{ij} D(i,j)(\partial I(i,j)/\partial U_k)$](./images/backoperator3.svg) and
![latex: \large $\sum_{ij} D(i,j)(\partial I(i,j)/\partial L_k)$](./images/backoperator4.svg)

with U<sub>k</sub> the texture coordinates of the kth vertex and L<sub>k</sub> its illumination coefficient. 
...
we can obtain the gradient with respect to the 2D vertices location and their colors using equation 1. 


In combination with automatic differentiation tool this core function allows to obtain the gradient of 
the error function with respect to the parameters of a a complex 3D scene we aim to estimate.


The rendering function implemented can draw image given a list of 2D projected triangles with associated depth (i.e 2.5D scene), where each triangle can have 
