...
class LaplacianRigidEnergyTensorflow:
    ...
    def __init__(self, mesh: ColoredTriMeshTensorflow, vertices: np.ndarray, cregu: float):
        self.numpy_imp = LaplacianRigidEnergy(mesh, vertices, cregu)
        self.cT_tf = scipy_sparse_matrix_to_tensorflow(self.numpy_imp.cT)
        self.approx_hessian = scipy_sparse_matrix_to_tensorflow(self.numpy_imp.approx_hessian)

    def evaluate(self, vertices: tf.Tensor) -> Tuple[tf.Tensor, tf.Tensor, tf.SparseTensor]:
        ...
        grad_vertices = tf.reshape(
            self.numpy_imp.cregu * tf.sparse.sparse_dense_matmul(self.cT_tf, diff[:, None]),
            vertices.shape,
