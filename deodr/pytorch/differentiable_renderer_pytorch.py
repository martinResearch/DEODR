...
class CameraPytorch(Camera):
    ...
    def world_to_camera(self, points_3d: torch.Tensor) -> torch.Tensor:
        ...
        return torch.cat((points_3d, torch.ones((points_3d.shape[0], 1), dtype=torch.double)), dim=1).mm(
            torch.tensor(self.extrinsic.T)
        )

    def left_mul_intrinsic(self, projected: torch.Tensor) -> torch.Tensor:
        return torch.cat((projected, torch.ones((projected.shape[0], 1), dtype=torch.double)), dim=1).mm(
            torch.tensor(self.intrinsic[:2, :].T)
        )

    def column_stack(self, values: Union[List[torch.Tensor], Tuple[torch.Tensor, ...]]) -> torch.Tensor:
...
class TorchDifferentiableRenderer2DFunc(torch.autograd.Function):
    ...
    def forward(  # type: ignore
        ...
        ctx.scene = scene
        scene.scene_2d.ij = ij.detach().numpy()  # should automatically detached according to
    ...
    def backward(ctx: Any, *grad_outputs: Any) -> Any:
        ...
        differentiable_renderer_cython.renderSceneBCpp(scene.scene_2d, 1, ctx.image, ctx.z_buffer, image_b.numpy())
        return (
...
class Scene3DPytorch(Scene3D):
    ...
    def set_light(self, light_directional: torch.Tensor, light_ambient: torch.Tensor) -> None:
    ...
    def _compute_vertices_colors_with_illumination(self) -> torch.Tensor:
        ...
        vertices_luminosity = (
            torch.relu(-torch.sum(self.mesh.vertex_normals * self.light_directional, dim=1))
