...
import numpy as np
from . import differentiablerenderer_cython
import torch
import copy
    ...
    def render_and_compare(self,sigma, Aobs, antialiaseError=False,mask=None,clear_gradients=True):
        ...
        if antialiaseError:
            ...
            if clear_gradients:
                self.clear_gradients()            
            ErrBuffer_B=None
            differentiablerenderer_cython.renderSceneB(self,sigma,Abuffer,Zbuffer,Abuffer_b,antialiaseError,Aobs,ErrBuffer,ErrBuffer_B)
...
class TorchDifferentiableRendererFunc(torch.autograd.Function):
    ...
    def forward(ctx,ij,colors,scene):
        ...
        differentiablerenderer_cython.renderScene(scene,1,Abuffer,Zbuffer)
    ...
    def backward(ctx, Abuffer_b):
        ...
        differentiablerenderer_cython.renderSceneB(scene,1,ctx.Abuffer,ctx.Zbuffer,Abuffer_b.numpy())         
