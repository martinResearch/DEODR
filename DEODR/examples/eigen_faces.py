...
def fun(points_deformed,pca_coefs):
    ...
    A_gt_zoomed = cv2.resize(A_gt, None, fx = rescale_factor, fy = rescale_factor, interpolation=cv2.INTER_NEAREST)
    Abuffer_zoomed = cv2.resize(Abuffer, None, fx = rescale_factor, fy = rescale_factor, interpolation=cv2.INTER_NEAREST)
    diffImage_zoomed = cv2.resize(diffImage, None, fx = rescale_factor, fy = rescale_factor, interpolation=cv2.INTER_NEAREST)
    
    cv2.polylines(A_gt_zoomed, (points_deformed_gt[tri.simplices]*64*rescale_factor).astype(np.int32), isClosed=True,color=(0,0,1)) 
    
    cv2.polylines(Abuffer_zoomed, (points_deformed[tri.simplices]*64*rescale_factor).astype(np.int32), isClosed=True,color=(0,0,1))   
...
for iter in range(nbIter):
    ...
    points_deformed[on_border]=points[on_border]
