...
def example_sfm(dl_library='pytorch', plot_curves=True, save_images=True, display = True):
    ...
    for idframe in range(nbFrames):
        handImages.append(np.roll(imread('hand.png').astype(np.double)/255,10*(idframe-1),axis=1))
    ...
    euler_init = np.tile(np.array([0.1,0.3,0.2])[None,:],[5,1])
    translation_init = np.zeros((nbFrames,3))
    ...
    for iter in range(maxIter):        
        ...
        if display or save_images:
            combinedIMage = np.column_stack((np.row_stack(handImages),np.row_stack(Abuffer),np.tile(np.row_stack(diffImage)[:,:,None],(1,1,3))))
            if display:
...
if __name__ == "__main__":
    display = True
    save_images = True
    example_sfm(dl_library='none', plot_curves = False, display = display, save_images = save_images)
    #example_sfm(dl_library='pytorch', plot_curves = False, display = display, save_images = save_images)    
    #example_sfm(dl_library='tensorflow', plot_curves = True, display = display, save_images = save_images)
