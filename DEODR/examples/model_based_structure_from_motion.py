...
def example_sfm(dl_library="pytorch", plot_curves=True, save_images=True, display=True):
    ...
    for idframe in range(nbFrames):
        handImages.append(
            np.roll(
                imread("hand.png").astype(np.double) / 255, 10 * (idframe - 1), axis=1
            )
        )
    ...
    euler_init = np.tile(np.array([0.1, 0.3, 0.2])[None, :], [nbFrames, 1])
        ...
        if display or save_images:
            combinedIMage = np.column_stack(
                (
                    ...
                    np.tile(np.row_stack(diffImage)[:, :, None], (1, 1, 3)),
                )
            )
            if display:
                cv2.imshow(
