...
def main():
    ...
    defaultLight = {
        "directional": np.array([0.1, -0.5, 0.5]),
        "ambiant": np.array([0.3]),
    }
    handFitter = DEODR.MeshRGBFitter(
        vertices, faces, defaultColor, defaultLight, cregu=4000
    )
    ...
    while True:
        ...
        while True:
            ...
            cv2.putText(
                frameCopy,
                "r: restart,  other key:starts and stop",
                (10, 20),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.5,
                (200, 0, 0),
                ...
                displayImage = (
                    cv2.resize(
                        np.column_stack(
                            (
                                handImage,
                                Abuffer.detach().numpy(),
                                np.tile(
                                    diffImage.detach().numpy()[:, :, None], (1, 1, 3)
                                ),
                            )
                        )[:, :, ::-1],
                        None,
