...
mesh=Mesh(vertices, faces[:,::-1].copy())#we do a copy to avoid negative stride not support by pytorch 
...
CameraMatrix=np.array([[focal,0,SizeW/2],[0,focal,SizeH/2],[0,0,1]]).dot(np.column_stack((R,T)))
