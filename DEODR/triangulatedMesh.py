...
class Mesh():
	...
	def computeFaceNormals(self):
		if isinstance(self.vertices,torch.Tensor):	
			...
			n = torch.cross( tris[::,1 ] - tris[::,0] ).cross( tris[::,2 ] - tris[::,0] )
		else:
			tris = self.vertices[self.faces,:]
			n = np.cross( tris[::,1 ] - tris[::,0] , tris[::,2 ] - tris[::,0] )
		self.faceNormals = n/(n**2).sum(dim=1)[:,None]

	def computeVertexNormals(self):
		self.computeFaceNormals()
		if isinstance(self.faceNormals,torch.Tensor):
			norm=self.Vertices_Faces_torch.mm(self.faceNormals)
		else:
			norm=self.Vertices_Faces*self.faceNormals
		norm,lens=normalize_v3(norm)
		self.vertexNormals = norm	
	...
	def computSilhouetteEddges(self,viewpoint):		
		...
		adjacent_visible_faces = (self.Edges_Faces_Ones*(np.arange(self.nbF)*(face_visible)))
		edges_list = np.nonzero(edge_bool)
		...
		self.adjacent_visible_faces=adjacent_visible_faces
