...
def TensorflowDifferentiableRender2D( ij, colors, scene):
    ...
    def forward(ij, colors):#using inner function as we don't differentate w.r.t scene
        ...
        def backward(Abuffer_b):            
            ...
            Abuffer_copy = Abuffer.copy()# making a copy to avoid removing antializaing on the image returned by the forward pass (the c++ backpropagation undo antializating), could be optional if we don't care about getting aliased images
            differentiable_renderer_cython.renderSceneB(scene, 1, Abuffer_copy, Zbuffer, Abuffer_b.numpy())         
