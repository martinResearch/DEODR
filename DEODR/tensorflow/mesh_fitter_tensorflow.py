...
class MeshRGBFitterWithPose():
    ...
    def step(self):
        ...
        with tf.GradientTape() as tape :
            ...
            self.mesh.setVerticesColors(tf.tile(handColor_with_grad[None,:], [self.mesh.nbV,1]))
           
            Abuffer = self.scene.render(self.CameraMatrix, resolution = (self.SizeW,self.SizeH))    
            
            diffImage = tf.reduce_sum((Abuffer-tf.constant(self.handImage))**2, axis = 2)
            loss = tf.reduce_sum(diffImage)        
            
            trainable_variables=[vertices_with_grad, quaternion_with_grad, translation_with_grad, ligthDirectional_with_grad, ambiantLight_with_grad, handColor_with_grad]
            vertices_grad, quaternion_grad, translation_grad, ligthDirectional_grad, ambiantLight_grad, handColor_grad = tape.gradient(loss,trainable_variables)
